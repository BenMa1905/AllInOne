...
const createSchedule = (req, res) => {
    ...
    console.log(newSchedule.startTime.getHours() * 60 + newSchedule.startTime.getMinutes())
    console.log(newSchedule.startTime.getDay() % 6 != 0)

    const endTimeMins = newSchedule.endTime.getHours() * 60 + newSchedule.endTime.getMinutes();
    console.log(endTimeMins)
    if ((newSchedule.startTime.getDay() % 6 != 0 && (startTimeMins < 450 || endTimeMins > 1260)) || (newSchedule.startTime.getDay() % 6 == 0 && startTimeMins < 480 || endTimeMins > 1320)) {
        return res.status(400).send({ message: "El agendamiento no se encuentra en horarios de antencion" })
    }

    Schedule.findOne({ washingMachine: newSchedule.washingMachine, $or: [{ startTime: { $gte: newSchedule.startTime, $lt: newSchedule.endTime } }, { endTime: { $gt: newSchedule.startTime, $lte: newSchedule.endTime } }] }, (errSche, fSchedule) => {
        if (errSche) {
            ...
            newSchedule.save((schError, nSchedule) => {
                ...
                Machine.updateMany({ $or: [{ _id: newSchedule.washingMachine, machineType: "washingMachine" }, { _id: newSchedule.dryingMachine, machineType: "dryingMachine" }] }, { $push: { schedule: newSchedule._id } }, (maErr, machines) => {
                    ...
                    Membership.findByIdAndUpdate(user.membership, { $inc: { remainingHours: -(rHours) } }, (memErr, membership) => {
                        ...
                        res.setHeader('Content-Type', 'application/json')
                        return res.status(200).send({ extras: extraInfo, createdSchedule: nSchedule })
                    })
...
const getDayMatrixSchedules = (req, res) => {
    const { date } = req.params
    const dateNumbers = date.toString().split('T')[0].split('-')
    const specificDate = new Date(parseInt(dateNumbers[0]), parseInt(dateNumbers[1]) - 1, parseInt(dateNumbers[2]));
    const endSpecificDate = new Date(parseInt(dateNumbers[0]), parseInt(dateNumbers[1]) - 1, parseInt(dateNumbers[2]) + 1);
    Machine.find({}, (err, machines) => {
        ...
        if (machines.length === 0) {
            return res.status(204).send({ message: "No se encontraron maquinas" })
        }


        Schedule.find({ startTime: { $gte: specificDate, $lt: endSpecificDate } }).populate({ path: 'washingMachine dryingMachine', select: 'serial' }).exec((error, schedules) => {
            if (error) {
                console.log(error)
                return res.status(400).send({ message: "No se pudo realizar la busqueda" })
            }

            return res.status(200).send({ allMachines: machines, daySchedules: schedules })
        })
    })
...
const getSchedules = (req, res) => {
    ...
    console.log(id)
    if (id != 'null') {
        Schedule.find({'user.objectIdField' :  ObjectId(id) }).populate({ path: 'user washingMachine dryingMachine', select: 'name serial' }).exec((error, schedules) => {
            if (error) {
                return res.status(400).send({ message: "No se pudo realizar la busqueda" })
            }
            if (schedules.length === 0) {
                return res.status(204).send({ message: "No se encontraron agendamientos" })
            }
            return res.status(200).send(schedules)
        })
    } else {
        Schedule.find({}).populate({ path: 'user washingMachine dryingMachine', select: 'name serial' }).exec((error, schedules) => {
            if (error) {
                return res.status(400).send({ message: "No se pudo realizar la busqueda" })
            }
            if (schedules.length === 0) {
...
const deleteSchedule = (req, res) => {
    ...
    if (opt == null) {
        auxRes = (cod, msg) => { return res.status(cod).send({ message: msg }) }
    ...
    Schedule.findByIdAndDelete(id, (error, schedule) => {
        ...
        if (!schedule) {
            return auxRes(401, "No se ha podido encontrar un agendamiento")
        }
        ...
        Machine.updateMany({ $or: [{ _id: schedule.washingMachine }, { _id: schedule.dryingMachine }] }, { $pull: { schedule: schedule._id } }, (error, machine) => {
            ...
            if (!machine) {
                return auxRes(402, "No se encontro la/s maquina/s")
            }
            User.findByIdAndUpdate(schedule.user, { $pull: { schedule: schedule._id } }, (error, user) => {
                ...
                if (!user) {
                    return auxRes(403, "No se encontro el usuario")
                }
...
const getSchedule = (req, res) => {
    ...
    Schedule.findById(id).populate({ path: 'user washingMachine dryingMachine', select: 'name serial' }).exec((error, schedule) => {
        if (error) {
